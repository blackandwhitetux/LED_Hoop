...
//int myLed  = 13;  // Set up pin 13 led for toggling  //not enough pins on esp-03
...
void setup()
{
  Wire.begin(15,16); //GPIO15 as SDA and GPIO16 as SCL
  ...
  //pinMode(myLed, OUTPUT); //not enough pins on esp-03
  //digitalWrite(myLed, HIGH); //not enough pins on esp-03
...
void loop() {
{
  ...
  if (!AHRS)
  {
    ...
    if (myIMU.delt_t > 500)
    {
      ...
      myIMU.count = millis();
      //digitalWrite(myLed, !digitalRead(myLed));  // toggle led  //not enough pins on esp-03
    } // if (myIMU.delt_t > 500)
  } // if (!AHRS)
  else
  {
